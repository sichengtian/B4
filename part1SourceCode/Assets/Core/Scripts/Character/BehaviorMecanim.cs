...
public class BehaviorMecanim : MonoBehaviour
{
    ...
    public Node Node_FightEachOther()
    {
        return 
            new Sequence(
                    new LeafInvoke(() => this.Character.LookAtEachOther()),
                    new SequenceParallel(
                    PooperBodyMovement("DUCK", 2000),
                    AgentHandMovement("HITSTEALTH", 2000)),
                    new LeafWait(1000)
            );
    }
    public Node Node_PistolAimEachOther()
    {
        return new Sequence(
                    new SequenceParallel(
                    PooperHandMovement("PISTOLAIM", 2000),
                    AgentHandMovement("PISTOLAIM", 2000))
            );
    }
    ...
    public Node PooperBodyMovement(Val<string> gestureName, Val<long> duration)
    {
        return new DecoratorCatch(
            () => this.Character.HandAnimation(gestureName, false),
            new Sequence(
                Node_BodyAnimation(gestureName, true),
                new LeafWait(duration),
                Node_BodyAnimation(gestureName, false)));
    }
    public Node AgentBodyMovement(Val<string> gestureName, Val<long> duration)
    {
        return new DecoratorCatch(
            () => this.Character.AgentHandAnimation(gestureName, false),
            new Sequence(
                Node_AgentBodyAnimation(gestureName, true),
                new LeafWait(duration),
                Node_AgentBodyAnimation(gestureName, false)));
    }
    ...
    public Node Node_EnableImage()
    {
        return new LeafInvoke(() =>
        {
            this.Character.EnableImage();
        });
    }
    public Node Node_DisableImage()
    {
        return new LeafInvoke(() =>
        {
            this.Character.DisableImage();
        });
    }
    ...
    public Node Node_AgentBodyAnimation(Val<string> gestureName, Val<bool> start)
    {
        return new LeafInvoke(
            () => this.Character.AgentBodyAnimation(gestureName, start),
            () => this.Character.AgentBodyAnimation(gestureName, false));
    }
    /// <summary>
